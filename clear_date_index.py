...
def generate_clear_index_new(power, cap):
    ...
    for group,frame in power.groupby(power.index.date):
        ...
        down= frame_plus_diff_down[frame_plus_diff_down['power']<0].count().tolist()[0]

        frame_plus_diff_up = frame_plus_diff[:middle].copy()
        up = frame_plus_diff_up[frame_plus_diff_up['power']>0].count().values.tolist()[0]
        ...
        judge = [1 if abs(frame_plus_diff.iloc[i].values)<0.005*cap else 0 for i in range(len(frame_plus_diff))]
        ...
        # requirement 4
        judge_4 = [1 if abs((frame_diff.iloc[i]/cap).tolist()[0]) > 0.2 else 0
        ...
        for i in range(10,70):
            if (frame.iloc[i]<frame.iloc[i-1]).tolist()[0] & (frame.iloc[i]<frame.iloc[i+1]).tolist()[0]:
                if (np.abs(frame_diff.iloc[i])>epison).values.tolist()[0] | (np.abs(frame_diff.iloc[i+1])>epison).values.tolist()[0]:
                    judge_5 = judge_5+1
                if (np.abs(frame_diff.iloc[i])>5*epison).values.tolist()[0] | (np.abs(frame_diff.iloc[i+1])>5*epison).values.tolist()[0]:
                    flag = 0
        if judge_5 >= wavenum:
            flag = 0
        label.append(flag)
        ...
        for i in range(middle-1):
            if np.abs((frame_plus.iloc[middle+i].values - frame_plus.iloc[middle-i].values)/cap) > limit:
                judge_6 = judge_6+1

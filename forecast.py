...
def func(I, a1, a2, a3): return (a1 + a2 * I + a3 * np.log(I)) * I
...
def forecast_(E, P_fit, power_, set_, ratio_test, K_Tc, start_time_k):
    ...
    E = pd.DataFrame(
        {'E': E[:len(power_)].values.reshape(-1)}, index=power_.index)
    ...
    for date in set_:
        ...
        if (date > set_[0]):
            ...
            popt_iter1, pcov_iter1 = curve_fit(
                func, X_fit_1.values.reshape(-1), Y_fit_1.values.reshape(-1), bounds=(lb, ub))

            P_I_iter_1 = np.array([0 if E.values.reshape(-1)[i] == 0 else func(
                E.values.reshape(-1)[i], *popt_iter1) for i in range(len(E))])
            P_I_iter_1[P_I_iter_1 < 0] = 0

            P_fit_iter_1 = P_I_iter_1 * \
                ratio_test[:len(power_)] * K_Tc[:len(power_)]
        ...
        #             frame_P_fit_iter_1_ = series_to_supervised(frame_P_fit_iter_1, 0, 16)
            ...
            P_fit_iter_1 = pd.DataFrame(
                {'P': P_fit[:len(power_)].values.reshape(-1)}, index=power_.index)
        ...
        for i in range(start_time_k, len(y_hat) - 1):
            ...
            ratio_fit = pd.DataFrame(
                {'ratio': ratio_test[:len(power_)]}, index=power_.index).loc[time_sunrise:time]
            K_Tc_fit = pd.DataFrame(
                {'K_Tc': K_Tc[:len(power_)]}, index=power_.index).loc[time_sunrise:time]
            Y_fit = frame.loc[time_sunrise:time].values.reshape(-1) / (
                ratio_fit.values.reshape(-1) * K_Tc_fit.values.reshape(-1))

            popt_iter, pcov_iter = curve_fit(
                func, X_fit.values.reshape(-1), Y_fit, bounds=(lb, ub))

            P_I_iter = np.array([0 if E.values.reshape(-1)[i] == 0 else func(
                E.values.reshape(-1)[i], *popt_iter) for i in range(len(E))])
            P_I_iter[P_I_iter < 0] = 0

            P_fit_iter = P_I_iter * \
                ratio_test[:len(power_)] * K_Tc[:len(power_)]
            P_fit_iter = pd.DataFrame(
                {'P_fit': P_fit_iter[:len(power_)]}, index=power_.index)

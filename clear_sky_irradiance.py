...
def clear_sky_model(latitude, lontitude):
    ...
    B = 360 * (n - 81) / 364
    B = B * np.pi / 180
    ET = 9.87 * np.sin(2 * B) - 7.53 * np.cos(B) - 1.5 * \
        np.sin(B)  # 校正因子，需要重复96次(一天96个点)
    l_st = 120
    l_local = lontitude
    ...
    sin_alpha = np.sin(fai) * np.sin(delta) + \
        np.cos(fai) * np.cos(delta) * np.cos(w)
    alpha = np.arcsin(sin_alpha) * 180 / np.pi

    # 大气光学质量
    m = 1 / (sin_alpha + 0.50572 * (np.sign(6.07995 + alpha)
                                    * (np.abs(6.07995 + alpha)) ** (-1.6364)))
    m = pd.DataFrame({'m': m})
    index = m[m['m'] <= 0].index
    # m.iloc[index] = 0
    m = m.values
    ...
    t_b_ = np.array([0.254, 0.285, 0.361, 0.401, 0.461, 0.545,
                     0.499, 0.440, 0.423, 0.401, 0.326, 0.261])
    t_d_ = np.array([2.415, 2.239, 1.997, 2.002, 1.875, 1.729,
                     1.925, 2.109, 2.035, 1.991, 2.186, 2.447])
